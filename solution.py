...
def validate(data):
    ...
    for record in data:
        ...
        if price is not None and price > 0 and category is not None and str(category).strip() != '':
            valid_records.append(record)
        else:
            error_count += 1

    print(f"Validation summary: {len(valid_records)} valid, {error_count} error records dropped.")
